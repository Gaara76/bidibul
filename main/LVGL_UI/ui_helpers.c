...
{
    if(id == _UI_LABEL_PROPERTY_TEXT) lv_label_set_text_fmt(target, val);
}
...
void _ui_screen_change(lv_obj_t **target, lv_scr_load_anim_t fademode, int spd, int delay, void (*target_init)(void))
{
    if (target == NULL || target_init == NULL) {
        ESP_LOGE("LVGL", "❌ Paramètres invalides (target ou target_init NULL) !");
        return;
    }

    if (*target == NULL) {
        ESP_LOGI("LVGL", "🛠️ Initialisation de l'écran cible...");
        target_init();

        // Vérifie après initialisation
        if (*target == NULL) {
            ESP_LOGE("LVGL", "❌ L'écran cible n'a pas été initialisé !");
            return;  // Empêche de charger un écran NULL
        }
    }

    // Vérifie que l'écran n'est pas déjà actif
    if (lv_scr_act() != *target) {
        lv_scr_load_anim(*target, fademode, spd, delay, false);
        lv_refr_now(NULL);  // Force rafraîchissement immédiat
        //ESP_LOGI("LVGL", "✅ Écran chargé avec succès.");
    }
}
...
{
    if(lv_obj_has_state(src, LV_STATE_CHECKED)) lv_label_set_text_fmt(trg, txt_on);
    else lv_label_set_text_fmt(trg, txt_off);
}

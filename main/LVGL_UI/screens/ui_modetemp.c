...
static void set_value_g(void *ui_Arc1, int32_t gite_acc)
 {
    ...
    float Accel_y = (float)filtered_accel_y / 30.0f; // précision
...
void ui_modetemp_screen_init(void)
{
    ...
    lv_anim_t anim;
    lv_anim_init(&anim);
    lv_anim_set_exec_cb(&anim, set_value_g); // Callback pour mettre à jour la valeur de la bille
    lv_anim_set_var(&anim, ui_Arc1); // L'indicateur à animer
    lv_anim_set_values(&anim, -70, 70);  // Plage de l'animation
    lv_anim_set_time(&anim, 500);  // Durée de l'animation
    lv_anim_set_repeat_delay(&anim, 0);  /*Délai avant la répétition. La valeur par défaut est 0 (désactivé) [ms]*/
    lv_anim_set_playback_time(&anim, 0);  /*Lorsque l'animation est prête, elle est jouée à l'envers avec cette durée. La valeur par défaut est 0 (désactivé) [ms]*/
    ...
    ///////////  INIT  HTML  /////////////
    vTaskDelay(pdMS_TO_TICKS(10000));  // Attendre que la connexion WiFi soit établie
    ...
    uint16_t port = 80;
    http_simple_get(host, path, port);  // Récupère la page et extrait les données
}
...
void http_simple_get(const char *host, const char *path, uint16_t port) {
    ...
    if (http_mutex == NULL) {
        ESP_LOGE(TAG, "❌ Mutex HTTP non initialisé !");
        return;
    }
    ...
    if (sock < 0) {
        ESP_LOGE(TAG, "❌ Impossible de créer le socket !");
        lv_label_set_text_fmt(ui_Label1, "N/A");
    ...
    if (err != 0) {
        ESP_LOGE(TAG, "❌ Échec connexion socket : errno %d", errno);
        lv_label_set_text_fmt(ui_Label1, "N/A");
        lv_obj_invalidate(ui_Label1);
        lv_label_set_text_fmt(ui_Label22, "N/A");
        lv_obj_invalidate(ui_Label22);
    ...
    if (err < 0) {
        ESP_LOGE(TAG, "❌ Erreur envoi requête !");
        close(sock);
        xSemaphoreGive(http_mutex);
        return;
    }

    //ESP_LOGI(TAG, "Requête envoyée, attente réponse...");
    ...
    if (total_read > 0) {
        ...
        if (json_start) {
            ...
            //ESP_LOGI(TAG, "🛬 Total reçu : %d octets", total_read);
            //ESP_LOGI(TAG, "Contenu JSON brut : %s", json_start);
            parse_json_data(json_start);
        } else {
            ESP_LOGE(TAG, "❌ Réponse HTTP invalide !");
        }
    } else {
        ESP_LOGE(TAG, "❌ Erreur réception ou réponse vide !");
...
void parse_json_data(const char *json) {
    if (json == NULL || strlen(json) == 0) {
        ESP_LOGE(TAG, "❌ Données JSON vides ou invalides !");
        lv_label_set_text_fmt(ui_Label1, "Err");
        lv_obj_invalidate(ui_Label1);
    ...
    //ESP_LOGI("HTTP_CLIENT", "Contenu reçu : %s", json);
    if (!root) {
    ...
    if (cJSON_IsArray(common_list)) {
            ...
            if (cJSON_IsString(id) && strcmp(id->valuestring, "0x0A") == 0) {
                ...
                if (cJSON_IsString(battery) && cJSON_IsString(voltage)) {
                    ...
                    if (compteur_pression == 0 && battery_level >= 1 && battery_level <= 5) {
                        ...
                        // Alerte spéciale si batterie faible
                        if (battery_level <= 2) {
                            ...
                            ESP_LOGW("BATTERY", "⚠️ Batterie faible !");
                        }
                    }
                }
            }
        ...
        ESP_LOGW(TAG, "⚠️ Aucune donnée `common_list` trouvée !");
        lv_label_set_text_fmt(ui_Label1, "N/A");
        lv_obj_invalidate(ui_Label1);
        lv_label_set_text_fmt(ui_Label25, "Min: N/A");
    ...
    if (cJSON_IsArray(wh25) && cJSON_GetArraySize(wh25) > 0) {
        ...
        if (cJSON_IsString(humidity)) {
            //ESP_LOGI(TAG, "💧 Humidité intérieure : %s", humidity->valuestring);
        }
        ...
        // Mise à jour température min/max
        ...
        if (compteur_minutes >= 235 || compteur_minutes == 1) {  // avec 78 points sur 24h
            //ESP_LOGI("BARO", "📊 Pression mesurée: %.2f hPa", pression_actuelle);
            //ESP_LOGI("BARO", "⏱️ Temps=%d", compteur_minutes);
            compteur_minutes = 1;
        ...
        ESP_LOGW(TAG, "⚠️ Aucune donnée `wh25` trouvée !");
        lv_label_set_text_fmt(ui_Label22, "N/A");
        lv_obj_invalidate(ui_Label22);
    }
...
void temperature_task(void *pvParameters) {
    ...
    while (1) {
        //ESP_LOGI(TAG, "Rafraîchissement des données météo...");
        http_simple_get(host, path, port);
        //ESP_LOGI(TAG, "Rafraîchissement terminé.");
...
void init_http_client() {
    ...
    //if (http_mutex == NULL) {
        //ESP_LOGE(TAG, "❌ Impossible de créer le mutex HTTP !");
    //} else {
        //ESP_LOGI(TAG, "Mutex HTTP initialisé avec succès.");
    //}
}
...
void lire_donnees_sdcard(void) {
    ...
    int numero_ligne = 0;
    ESP_LOGI(TAG, "🔍 Lecture du fichier...");
    while (fgets(ligne_buffer, sizeof(ligne_buffer), f) != NULL) {
        ...
        if (sscanf(ligne_buffer, "%f,%f", &temperature_max, &temperature_min) == 2) {
            //ESP_LOGI(TAG, "Ligne %d → temperature_max: %.1f°C, temperature_min: %.1f °C",
            //         ++numero_ligne, temperature_max, temperature_min);
        } else {
            ESP_LOGW(TAG, "⚠️ Ligne %d invalide", ++numero_ligne);
            temperature_max = -100.0f;
            temperature_min = 100.0f;
        }
    }
    ESP_LOGI(TAG, "Lecture du fichier terminé.");
    fclose(f);
}
